...
from models.discriminator import RealFakeDiscriminator, DoubleAMCDiscriminator
from models.generator import Generator,Generator_Sampling
...
class FreqLoss(nn.Module):
    ...
    def forward(self, x, y):
        loss = 0
        for kpts in range(20):
            x_freq = self.spectrogram(x[:,:,kpts])
            y_freq = self.spectrogram(y[:,:,kpts])
            loss += self.mse(x_freq,y_freq)
        return loss
...
if __name__ == '__main__':
    ...
    AMCNet_perceptual = torch.load('checkpoints/Dc_globalstep87000.pt').cuda().pose_encoder
    ...
    G.music_encoder = torch.load('checkpoints/Dc_globalstep87000.pt').cuda().music_encoder
    ...
    for e in range(epoch_num):
        ...
        for step, (batch_x, batch_y) in pbar:
            ...
            loss_freq = Freq_criterion(fake_pose, var_y)

            if global_step <1000:
                loss_G = 1 * loss_pred + 0 * loss_Gadv_Dr + 0 * loss_perceptual + 0 * loss_freq
            else:
                loss_G = 0 * loss_pred + 0.0005 * loss_Gadv_Dr + 0.1 * loss_perceptual + 0 * loss_freq
                # loss_G = 0 * loss_pred + 0 * loss_Gadv_Dr + 1 * loss_perceptual + 0 * loss_freq
            loss_G.backward()
            ...
            writer.add_scalars('1. Generator loss', {'MSE':loss_pred.item(),
                                                    'Freq':loss_freq.item()}, global_step)
            writer.add_scalars('1. gradient', {'G': total_norm_G}, global_step)
            ...
            writer.add_scalars('3.variences',{'GT':torch.var(var_y),
                                              'pred':torch.var(fake_pose)},global_step)
            '''
