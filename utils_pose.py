...
def show_pose(keypoints, descriptions, name, video_save_dir=None):
    ...
    if video_save_dir is not None:
        wirter = cv2.VideoWriter(video_save_dir + name + '.avi', 0, cv2.VideoWriter_fourcc(*'XVID'), 30, (int(num_fig * figsize), figsize))
    ...
    for frame in tqdm.tqdm(range(num_frame)):
        ...
        for numfig in range(num_fig):
            ...
            # --- put text --- #
            cv2.putText(img, descriptions[numfig], (numfig * figsize+5, 30), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (50, 50, 50), 1)
            cv2.putText(img, time_stamp, (numfig * figsize+5, 60), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (50, 50, 50), 1)
            cv2.putText(img, 'frame '+str(frame), (numfig * figsize+5, 90), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (50, 50, 50), 1)
            cv2.putText(img, name, (numfig * figsize+5, 120), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (50, 50, 50), 1)
...
if __name__ == '__main__':
    ...
    for name in name_list:
        ...
        keypoints = read_result(name, result_dir=result_dir)

        high_pass = filter(keypoints, mode='high pass')
        low_pass = filter(keypoints, mode='low pass')
        ...
        show_pose([keypoints,high_pass,low_pass],['alphapose','highpass','lowpass'])
